...
public class ArrClient(string host, string apiKey)
{
    ...
    public string Host { get; } = host.Trim().TrimEnd('/');
    ...
    public Task<HttpStatusCode> DeleteQueueRecord(
        int id, ArrConfig.QueueAction request, CancellationToken ct = default) =>
    ...
    private Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken ct = default)
    {
