...
internal static class ConfigUpdateValidator
{
    ...
    {
        if (!SettingsRegistry.TryGetValidationDefault(key, out var defaultValue))
        ...
        if (isInteger)
        {
            ...
            if (SettingsRegistry.Ranges.TryGetValue(key, out var range)
                ...
                Invalid($"{key} must be between {range.Min} and {range.Max}.");
        }

        if (SettingsRegistry.Choices.TryGetValue(key, out var choices)
            && !choices.Contains(value, StringComparer.OrdinalIgnoreCase))
            Invalid($"{key} must be one of: {string.Join(", ", choices)}.");
    }
