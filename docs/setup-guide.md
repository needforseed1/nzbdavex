...
3. **NzbDavEx** tells Radarr the "download" is finished and points to a folder of **Symlinks** at `/mnt/remote/davex/completed-symlinks`.
    * The **Symlinks** always point to the `/mnt/remote/davex/.ids` folder which contains the on-demand file content.
...
2. **NzbDavEx Internal Test:**
   ...
   * Now construct test command like below and run it in another terminal window:
     ```bash
     docker exec davex sh -c "apk add --no-cache wget > /dev/null 2>&1 && timeout 20s wget -O /dev/null --report-speed=bits --progress=bar:force:noscroll 'http://localhost:8080/view/content/Movies/<Movie Folder>/<Movie Name>.mkv?downloadKey=<download-key>'"
     ```
...
Now we mount the NzbDavEx web dav to the host file system using a sidecar container.

### 1. Prepare Host Directory

```bash
sudo mkdir -p /mnt/remote/davex # Create mount folder
sudo chown -R $(id -u):$(id -g) -R /mnt/remote/davex # Give ownership of the folder to your user
```
...
*Generate obscured password:* `docker run --rm -it rclone/rclone obscure "<the-webdav-password-you-set-in-davex-earlier>"`
...
1. **Mount Directory (`Settings` > `SABnzbd`):**
   * **Rclone Mount Directory:** `/mnt/remote/davex`
...
2. **URL:** `http://davex:3000/adapters/newznab/{token}` (substitute your profile's token; do **not** include `/api` here — Prowlarr appends it).
...
```
http://davex:3000/adapters/addon/{token}/manifest.json
```
  ...
  "results": [
    {
      ...
      "grabs": 42,
      "playUrl": "http://davex:3000/api/search/{token}/play/{playToken}.mkv"
    }
  ]
}
```
